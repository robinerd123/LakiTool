...
namespace LakiTool
{
    class LUTUtil
    {
        ...
        public Labels.LabelContainer jumpContainer = new Labels.LabelContainer();

        string[] vals = new string[0];

        public void initF3D(SpecialRendering specialRendering = SpecialRendering.Off)
        {
            int n = -1;
            foreach (string line in dldata)
            {
                ...
                {
                    jumpContainer.labels.Add(Labels.Utils.LabelUtil.findLabelFromName(vals[1], curFile, MISC.Game.gamePath, true, true));
                }
